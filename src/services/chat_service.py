    ...
    def ask(
        ...
        if response_type == "dual":
            ...
            self.stat_repo.create(
                MessageStat(message=rag_resp, word_count=len(rag_answer.split()))
            )
            ...
            graph_citation_objs = self._build_graph_citations(
                graph_citations_raw, resp_msg=graph_resp
            )

            # Lưu thống kê
            self.stat_repo.create(
                MessageStat(message=graph_resp, word_count=len(graph_answer.split()))
            )
        ...
        if response_type == "rag":
            ...
            citations_objs = self._build_rag_citations(
                retrieved_docs, resp_msg=resp_msg
            )
        ...
        if response_type == "graph_rag":
            ...
            # Lưu citations vào DB với resp_msg đã có (chỉ gọi 1 lần)
            citations_objs = self._build_graph_citations(
                citations_raw, resp_msg=resp_msg
            )
                ...
                "citations": [
                    {
                        "marker": (
                            c.citation_marker
                            if hasattr(c, "citation_marker")
                            else c.get("marker")
                        ),
                        "graph_entity_name": (
                            c.graph_entity_name
                            if hasattr(c, "graph_entity_name")
                            else c.get("graph_entity_name")
                        ),
                        "graph_entity_type": (
                            c.graph_entity_type
                            if hasattr(c, "graph_entity_type")
                            else c.get("graph_entity_type")
                        ),
                    }
                    for c in citations_objs
                ],
