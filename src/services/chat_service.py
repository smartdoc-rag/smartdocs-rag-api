...
class ChatService:
    ...
    def _retrieve_chunks_filtered(self, conversation_id, query, top_k=5, selected_file_ids=None, search_type="vector"):
        if search_type == "hybrid":
            ...
            if selected_file_ids:
                ...
                for fid in selected_file_ids:
                    docs = vectorstore.similarity_search(query, k=top_k, filter={"file_id": str(fid)})
    ...
    def ask(self, conversation_id: int, user_id: int, question: str,
        ...
        if response_type == "rag":
            retrieved_docs = self._retrieve_chunks_filtered(
                conversation_id, question, top_k=top_k * 2 if use_reranking else top_k,
                selected_file_ids=selected_file_ids, search_type=search_type
            )
            if use_reranking and retrieved_docs:
                retrieved_docs = self._rerank_docs(question, retrieved_docs, top_k=top_k)
        ...
        if use_reranking and retrieved_docs:
            retrieved_docs = self._rerank_docs(final_question, retrieved_docs, top_k=top_k)
        elif use_self_rag and not use_reranking:
            retrieved_docs = retrieved_docs[:top_k]

        answer = rag.chat_flow(final_question, context_docs=retrieved_docs, chat_history=chat_history)
        ...
        if use_self_rag:
            ...
            if rag.needs_more_info(final_question, answer, confidence):
                ...
                if extra_docs:
                    ...
                    answer = rag.chat_flow(final_question, context_docs=unique_docs[:top_k * 2],
        ...
        for doc in retrieved_docs:
            ...
            if file_id:
                ...
                if file_obj:
                    ...
                    citation_marker = self._format_citation_marker(citation_counter, start_line, end_line)
    ...
    def _get_hybrid_retriever(self, conversation_id: int, top_k: int = 5, weights: tuple = (0.5, 0.5)):
        vectorstore = self._load_vectorstore(conversation_id)
        if not vectorstore:
            return None
        ...
        ensemble = EnsembleRetriever(retrievers=[vector_retriever, bm25_retriever], weights=weights)
        return ensemble
    ...
    def _format_citation_marker(self, citation_id: int, start_line: int, end_line: int) -> str:
